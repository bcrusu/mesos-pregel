...
type GraphStore interface {
	...
	LoadVertices(vranges []VertexRange) ([]*pregel.Vertex, error)

	SaveEdges(edges []*pregel.Edge) error
	LoadEdges(vranges []VertexRange) ([]*pregel.Edge, error)

	LoadVertexMessages(jobID string, superstep int, vranges []VertexRange) ([]*pregel.VertexMessage, error)
	SaveVertexMessages(messages []*pregel.VertexMessage) error

	LoadVertexOperations(jobID string, superstep int, vranges []VertexRange) ([]*pregel.VertexOperation, error)
	SaveVertexOperations(operations []*pregel.VertexOperation) error

	LoadHaltedVertices(jobID string, superstep int, vranges []VertexRange) ([]string, error)
	SaveHaltedVertices(halted []*pregel.VertexHalted) error

	LoadEdgeOperations(jobID string, superstep int, vranges []VertexRange) ([]*pregel.EdgeOperation, error)
